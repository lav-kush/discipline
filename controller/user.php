<?php
    ...
    class user {
        ...
        function signup($arguments) {
            ...
            // $len = count($data);
            loadView('header', array_merge($this->data, ['title' => 'SignUp - Discipline']));
        ...
        function send_otp($arguments) {
            ...
            if (!isset($_POST) || empty($_POST) || !isset($_POST['username']) || empty($_POST['username']) || !isset($_POST['email']) || empty($_POST['email']) || !isset($_POST['name']) || empty($_POST['name']) || !isset($_POST['fname']) || empty($_POST['fname'])) {
            ...
            try {
                ...
                $mail->SMTPSecure = 'tls';
                $mail->Port = 587;
                $mail->setFrom('codeshows123@gmail.com', 'Discipline');
                $mail->addAddress($arguments['email'], $arguments['name']." ".$arguments['fname']);
                $mail->isHTML(true);
                $mail->Subject = 'Discipline - OTP Verification';
                $mail->Body    = $body;
                $mail->AltBody = $body;
                $mail->send();
            } catch (Exception $e) {
        ...
        function forgot_password($arguments) {
            ...
            try {
                ...
                $mail->SMTPSecure = 'tls';
                $mail->Port = 587;
                $mail->setFrom('codeshows123@gmail.com', 'Discipline, Udaipur');
                $mail->addAddress($result['email'], $result['name']." ".$result['fname']);
                $mail->isHTML(true);
                $mail->Subject = 'Discipline - Password Reset Instructions '.$result['username'];
                $url = 'http';
        ...
        function forgot_verify($arguments) {
            ...
            loadView('header', ['title' => 'Discipline - Change Password']);
        ...
        function subscribe($arguments) {
         ...
         function notice($arguments) {
            $result = loadModel('user', 'notice', $arguments);
            if ($result === false) {
                print('Some Error Occured<br/>Please Try Again');
                loadView('error', ['msg' => 'Some Error Occured']);
                exit();
        ...
        function add_banner($arguments) {
            $result = loadModel('user', 'add_banner', $arguments);
            if ($result === false) {
                print('Some Error Occured<br/>Please Try Again');
                loadView('error', ['msg' => 'Some Error Occured']);
                exit();
        ...
        function delete_banner($arguments) {
            $result = loadModel('user', 'delete_banner', $arguments);
            if ($result === false) {
                print('Some Error Occured<br/>Please Try Again');
                loadView('error', ['msg' => 'Some Error Occured']);
                exit();
        ...
        function add_notification($arguments) {
            $result = loadModel('user', 'add_notification', $arguments);
            if ($result === false) {
                print('Some Error Occured<br/>Please Try Again');
                loadView('error', ['msg' => 'Some Error Occured']);
                exit();
        ...
        function delete_notification($arguments) {
            $result = loadModel('user', 'delete_notification', $arguments);
            if ($result === false) {
                print('Some Error Occured<br/>Please Try Again');
                loadView('error', ['msg' => 'Some Error Occured']);
                exit();
        ...
        function profile($arguments) {
            ...
            loadView("header", array_merge($this->data, ['title' => $data[0]['username']." - Discipline"]));
